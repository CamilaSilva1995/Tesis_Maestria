...
De aqui en adelante solo trabajaremos con los datos filtrados por calidad, ya que no vale la pena continuar con los datos que contiene  muestras en ceros, ya que esto altera las visualizaciones.
...
```{r}
library(dplyr)
# Calcular la abundancia promedio por Phylum
average_abundance <- percentages_df_phylum %>%
     group_by(Phylum) %>%
...
```{r}
# Filtrar los datos para seleccionar únicamente el filo "Oomycota"
oomycota_abundance <- percentages_df_phylum %>%
  filter(Phylum == "Oomycota") %>%
  group_by(Treatment) %>%
  summarise(total_abundance = sum(Abundance, na.rm = TRUE), # Abundancia total
            mean_abundance = mean(Abundance, na.rm = TRUE), # Abundancia promedio
            sample_count = n()) # Número de muestras con Oomycota

oomycota_abundance
```

```{r}
# Calcular abundancia total, promedio y número de muestras para cada filo por tratamiento
microorganism_summary <- percentages_df_phylum %>%
  group_by(Phylum, Treatment) %>%
  summarise(
    total_abundance = sum(Abundance, na.rm = TRUE), # Abundancia total por filo y tratamiento
    mean_abundance = mean(Abundance, na.rm = TRUE), # Abundancia promedio por filo y tratamiento
    sample_count = n() # Número de muestras que contienen el filo
  ) %>%
  arrange(desc(total_abundance)) # Ordenar por abundancia total
...
```{r}
 # Agrupar por género (Genus) y tratamiento (Treatment)
 critical_genera <- percentages_Eukaryota_df %>%
     group_by(Genus, Treatment) %>%
     summarise(
         total_abundance = sum(Abundance, na.rm = TRUE), # Suma total de abundancia
         mean_abundance = mean(Abundance, na.rm = TRUE), # Abundancia promedio
         sample_count = n() # Número de muestras que contienen el género
     ) %>%
     ungroup() %>%
     arrange(desc(total_abundance)) # Ordenar por abundancia total
 ...
 # Calcular diferencia de abundancia promedio entre tratamientos

 # Crear una tabla separada para cada tratamiento
 healthy_data <- critical_genera %>% filter(Treatment == "healthy") %>%
     rename(total_abundance_healthy = total_abundance, mean_abundance_healthy = mean_abundance)
 
 wilted_data <- critical_genera %>% filter(Treatment == "wilted") %>%
     rename(total_abundance_wilted = total_abundance, mean_abundance_wilted = mean_abundance)
 
 # Combinar las tablas por el género
 critical_genera_diff <- merge(healthy_data, wilted_data, by = "Genus", all = TRUE)
 
 # Calcular la diferencia de abundancia promedio
 critical_genera_diff <- critical_genera_diff %>%
     mutate(
         abundance_diff = mean_abundance_healthy - mean_abundance_wilted
     ) %>%
     arrange(desc(abs(abundance_diff))) # Ordenar por la diferencia más alta
 
 # Mostrar los géneros más afectados
 head(critical_genera_diff, 10)
```
